    ...
    for (j = 0; j < demoClasses_; ++j) {
      if (dets[i].prob[j]) {
        ...
        if (BoundingBox_width > 0.01 && BoundingBox_height > 0.01) {
          ...
          //输出x和y方向坐标
		  bool ObjectFlat = false;

		  //ObjectFlat += !strcmp(demoNames_[j], "tvmonitor");
          	  //ObjectFlat += !strcmp(demoNames_[j], "person");
		  //ObjectFlat += !strcmp(demoNames_[j], "bicycle");
		  ObjectFlat += !strcmp(demoNames_[j], "car");
		  //ObjectFlat += !strcmp(demoNames_[j], "motorbike");
		  //ObjectFlat += !strcmp(demoNames_[j], "bus");
		  //ObjectFlat += !strcmp(demoNames_[j], "truck");

		  if(ObjectFlat){
            printf("%d, object: %s, porb: %d, x: %f, y: %f\n", count, demoNames_[j], int(dets[i].prob[j]*100), x_center, y_center);
		  }
    ...
    for (int i = 0; i < numClasses_; i++) {
      if (rosBoxCounter_[i] > 0) {
        ...
        for (int j = 0; j < rosBoxCounter_[i]; j++) {
          ...
          //printf("classLabels: %s, prob: %d\n", demoNames_[classLabels_[i]], int(rosBoxes_[i][j].prob * 100));
        }
      }
    }
